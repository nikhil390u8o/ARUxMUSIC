...
Admin commands:
  ...
  /setusername <username>  - Owner username set karo (start/ping/play me dikhega).
...
# /setusername
@bot.on_message(filters.command("setusername") & filters.private)
async def set_owner_username(client, msg: Message):
    if not _is_owner(msg.from_user.id):
        return await msg.reply_text("❌ <b>Sirf owner ye command use kar sakta hai!</b>")
    parts = msg.text.split(maxsplit=1)
    if len(parts) < 2:
        return await msg.reply_text(
            "⚠️ <b>Usage:</b> <code>/setusername yourusername</code>\n\n"
            "• @ mat lagao, sirf username likho\n"
            "• Ex: <code>/setusername ll_PANDA_BBY_ll</code>"
        )
    username = parts[1].strip().lstrip("@")
    S.set(config.BOT_TOKEN, "owner_username", username)
    await msg.reply_text(
        f"✅ <b>Owner username set!</b>\n\n"
        f"👤 <code>@{username}</code>\n\n"
        f"Ab /start, /ping aur play panel me yahi username dikhega."
    )
...
async def set_string(client, msg: Message):
    ...
    m = await msg.reply_text("⏳ <b>Connecting new assistant...</b>")

    try:
        from ARUMUZIC.clients import reload_assistant
        await reload_assistant(session)
        S.set(config.BOT_TOKEN, "session_string", session)
        await m.edit_text(
            "✅ <b>Assistant successfully connect ho gaya!</b>\n\n"
            "🎵 Ab VC mein music play kar sakte ho.\n"
            "⚠️ <i>String kisi ko mat dikhana — ye tera account hai!</i>"
        )
    except Exception as e:
        await m.edit_text(
            f"❌ <b>Connect fail hua!</b>\n\n"
            f"<code>{e}</code>\n\n"
            f"String sahi hai? Dobara check karo."
        )
