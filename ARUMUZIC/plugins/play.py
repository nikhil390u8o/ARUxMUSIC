...
def get_player_buttons(duration, elapsed=0, token=None):
    # Dynamic settings — har bot apna owner/support use karta hai
    _token       = token or config.BOT_TOKEN
    cfg          = S.get_all(_token)
    support_link = cfg.get("support_link", "https://t.me/sxyaru")
    owner_id     = cfg.get("owner_id") or config.CLONE_OWNER_ID
    owner_name   = cfg.get("owner_username") or config.CLONE_OWNER_USERNAME or "sxyaru"
    owner_url    = f"tg://user?id={owner_id}" if owner_id else f"https://t.me/{owner_name}"
...
async def play_next(chat_id: int):
    ...
    try:
        ...
        text = (
            ...
            f"<b>‣ ᴀᴘɪ ʙʏ: <a href='{owner_url}'>{owner_name}</a></b>\n"
            f"<b>‣ ᴀᴘɪ ᴍᴀᴅᴇ ʙʏ: <a href='{owner_url}'>{owner_name}</a></b>"
        )
        
        pmp = await bot.send_photo(chat_id, photo=thumbnail, caption=text, reply_markup=get_player_buttons(duration, token=_token))
